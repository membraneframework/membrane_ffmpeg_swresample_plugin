defmodule Membrane.Element.FFmpeg.SWResample.ConverterSpec do
  ...
  describe ".handle_caps/3" do
    ...
    it "should initialize native handle with receied sink_caps" do
      expect(described_module().handle_caps(:sink, sink_caps(), state()))
      |> to(eq {:ok, [{:caps, {:source, src_caps()}}], %{state() | native: <<>>}})
    end
  end
  ...
  describe ".handle_buffer/4" do
    ...
    let! :native do
      {:ok, native} =
        ConverterNative.create(
          sink_caps().format |> Caps.SerializedFormat.from_atom(),
          sink_caps().sample_rate,
          sink_caps().channels,
          src_caps().format |> Caps.SerializedFormat.from_atom(),
          src_caps().sample_rate,
          src_caps().channels
        )
    ...
    describe "in usual case" do
      ...
      it "should convert data properly" do
        {:ok, [{:send, {:source, %Buffer{payload: result}}}], state} =
    ...
    describe "if data size is too small to process" do
      ...
      describe "if data is empty" do
        ...
        it "should return ok result and not send anything forwards" do
          expect(described_module().handle_buffer(:sink, sink_caps(), buffer(), state()))
          |> to(eq {:ok, state()})
        end
      end
